...
class OutcomeForm(Form):
    accounting_main = forms.ModelChoiceField(label='Compte principal',
                                             queryset=Main.objects.filter(account_type='decaissement'),
    ...
    slip_number = forms.CharField(label='Numéro bordereau', widget=forms.widgets.TextInput(
        attrs={'id': 'slip-number-field', 'class': 'form-control',
               'placeholder': 'Entrez le numéro de bordereau'}))
    ...
    more = forms.CharField(label="Description",
                           widget=forms.widgets.TextInput(
                               attrs={'placeholder': 'Entrez une description', 'class': 'form-control'}))


class IncomeForm(Form):
    accounting_main = forms.ModelChoiceField(label='Compte principal', required=True,
                                             queryset=Main.objects.filter(account_type='encaissement'),
                                             empty_label='Entrez un compte principal',
                                             widget=forms.widgets.Select(
                                                 attrs={'class': 'form-select', 'data-choices': True,
                                                        'id': 'accounting-field'}))

    slip_number = forms.CharField(label='Numéro bordereau', required=False, widget=forms.widgets.TextInput(
        attrs={'id': 'slip-number-field', 'class': 'form-control',
               'placeholder': 'Entrez le numéro de bordereau'}))
    ...
    more = forms.CharField(label="Description", required=False,
                           widget=forms.widgets.TextInput(
                               attrs={'placeholder': 'Entrez une description', 'class': 'form-control'}))


class IncomeModelForm(ModelForm):
    class Meta:
        model = Income
        fields = ['accounting_main', 'amount', 'in_at']
        labels = {
            'accounting_main': 'Compte principal',
            'slip_number': '# de bordereau',
            'amount': 'Montant',
            'in_at': "Date de l'entrée",
            'more': 'Autres détails'
        }
        widgets = {
            'accounting_main': Select(attrs={'id': 'accounting-field', 'class': 'form-select mb-3',
